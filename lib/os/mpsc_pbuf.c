			...
			union mpsc_pbuf_generic item)
{
	...
	do {
		...
		} else if (buffer->flags & MPSC_PBUF_MODE_OVERFLOW) {
					 ...
					 size_t wlen, k_timeout_t timeout)
{
	...
	do {
		...
		k_spinlock_key_t key = k_spin_lock(&buffer->lock);
	        bool wrap = free_space(buffer, &free_wlen);
...
void mpsc_pbuf_commit(struct mpsc_pbuf_buffer *buffer,
		       union mpsc_pbuf_generic *item)
{
	uint32_t wlen = buffer->get_len(item);
...
union mpsc_pbuf_generic *mpsc_pbuf_claim(struct mpsc_pbuf_buffer *buffer)
{
	...
	do {
		...
		item = (union mpsc_pbuf_generic *)&buffer->buf[buffer->tmp_rd_idx];
		...
		if (!a || is_invalid(item)) {
			...
			if (skip || !is_valid(item)) {
				uint32_t inc =
					skip ? skip : buffer->get_len(item);

				buffer->tmp_rd_idx =
				      idx_inc(buffer, buffer->tmp_rd_idx, inc);
				buffer->rd_idx = idx_inc(buffer, buffer->rd_idx, inc);
				cont = true;
			} else {
				item->hdr.busy = 1;
				buffer->tmp_rd_idx = idx_inc(buffer, buffer->tmp_rd_idx,
							buffer->get_len(item));
			}
		}
		     ...
		     union mpsc_pbuf_generic *item)
{
	...
	item->hdr.valid = 0;
	if (!(buffer->flags & MPSC_PBUF_MODE_OVERFLOW) ||
