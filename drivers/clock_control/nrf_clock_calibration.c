...
static void hf_request(void)
{
	...
	clk_request(COND_CODE_1(NRF_CLOCK_HAS_HFCLK,
				DEVICE_DT_GET_ONE(nordic_nrfx_clock_hfclk),
				DEVICE_DT_GET_ONE(nordic_nrfx_clock_xo)),
...
#else
	clk_request(DEVICE_DT_GET_ONE(nordic_nrfx_clock_lfclk), &client, cal_lf_callback);
#endif
}
...
#else
	clk_release(DEVICE_DT_GET_ONE(nordic_nrfx_clock_lfclk));
#endif
}
