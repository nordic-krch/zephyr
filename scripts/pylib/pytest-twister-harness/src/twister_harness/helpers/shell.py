...
class Shell:
    ...
    def exec_command(
        ...
        *,
        ...
        if get_full_output:
            drain_budget = (
                full_output_timeout if full_output_timeout is not None else timeout
            )
            lines.extend(
                self._read_lines_for_duration(
                    drain_budget, print_output=print_output
                )
            )
        return lines

    def _read_lines_for_duration(
        self, duration_s: float, print_output: bool
    ) -> list[str]:
        ...
        while time.time() < deadline:
            ...
            try:
                line = self._device.readline(
                    timeout=min(0.2, remaining), print_output=print_output
                )
                lines.append(line)
            except TwisterHarnessTimeoutException:
                pass
