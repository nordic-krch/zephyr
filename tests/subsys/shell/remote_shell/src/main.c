...
static void get_remote_names(void)
{
	STRUCT_SECTION_FOREACH(shell_remote, remote) {
		...
		remote_names[remote_names_count] = (char *)malloc(remote_name_len + 1);
		memcpy((void *)remote_names[remote_names_count], remote->name, remote_name_len);
		remote_names[remote_names_count][remote_name_len] = '\0';
		remote_names_count++;
	}
				...
				size_t expected_len)
{
	...
	zassert_equal(len, expected_len,
			"len: %d, expected_len:%d response:%s", len, expected_len, str);
	zassert_mem_equal(str, expected, expected_len,
			"cmd: %s, got:%s, expected:%s",
			cmd, str, expected);
	shell_backend_dummy_clear_output(sh);
}

ZTEST(remote_shell, test_local_cmd)
{
	#define PING_RESP "\r\npong\r\n"
...
ZTEST(remote_shell, test_remote_ping)
{
	#define PING_RESP "\r\npong\r\n"
...
ZTEST(remote_shell, test_local_tab)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int rv;
	size_t len;
...
ZTEST(remote_shell, test_remote_tab)
{
	#undef TAB_COMMAND
...
ZTEST(remote_shell, test_remote_cmd_tab)
{
	#undef TAB_COMMAND
...
ZTEST(remote_shell, test_remote_too_long_print_message)
{
...
ZTEST(remote_shell, test_remote_static_cmd3_error_warn)
{
	...
	const char *str = shell_backend_dummy_get_output(sh, &len);
	zassert_not_null(strstr(str, "remote_static_cmd3_err"),
			"error output missing: %s", str);
	zassert_not_null(strstr(str, "remote_static_cmd3_warn"),
			"warning output missing: %s", str);
}

ZTEST(remote_shell, test_remote_dyn_cmd_tab)
{
	#undef TAB_COMMAND
...
ZTEST(remote_shell, test_remote_execute_cmd2)
{
...
ZTEST(remote_shell, test_remote_dynamic_subcmd)
{
	#define DYN_RESP "\r\nsub_cmd_1\r\n"
...
ZTEST(remote_shell, test_remote_print_uint64_t)
{
...
ZTEST_SUITE(remote_shell, NULL, remote_shell_setup, setup, NULL, NULL);
