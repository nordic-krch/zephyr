...
ZTEST(_ipc_service_benchmark, test_turnaround_time_16_bytes)
{
...
{
	tx_performance(__func__, &tdata[0], 32, 100, false);
}
...
ZTEST(ipc_service_benchmark, test_tx_32_performance_no_copy)
{
	...
	tx_performance(__func__, &tdata[0], 32, 100, true);
}

ZTEST(ipc_service_benchmark, test_tx_64_performance)
{
	tx_performance(__func__, &tdata[0], 64, 100, false);
}
...
ZTEST(ipc_service_benchmark, test_tx_64_performance_no_copy)
{
	...
	tx_performance(__func__, &tdata[0], 64, 100, true);
}

ZTEST(ipc_service_benchmark, test_tx_128_performance)
{
	tx_performance(__func__, &tdata[0], 128, 100, false);
}
...
ZTEST(ipc_service_benchmark, test_tx_128_performance_no_copy)
{
	...
	tx_performance(__func__, &tdata[0], 128, 100, true);
}
...
static void ep_recv_stress(const void *data, size_t len, void *priv)
{
	...
	for (size_t i = 0; i < dlen; i++) {
		if (pkt_data[i] != (uint8_t)(rx_cnt + i)) {
			err = true;
			uint32_t *d32 = data;
			LOG_HEXDUMP_INF(&d32[-2], len + 8, "packet");
			LOG_ERR("pkt:%p", data);
			LOG_ERR("prio:%d, pkt:%d bytes, rx_cnt0:%d(%02x) "
				"rx_cnt1:%d(%02x) rx_cnt2:%d(%02x)",
					prio, len,
					tdata->stress.ctx[0].rx_cnt,
					tdata->stress.ctx[0].rx_cnt,
					tdata->stress.ctx[1].rx_cnt,
					tdata->stress.ctx[1].rx_cnt,
					tdata->stress.ctx[2].rx_cnt,
					tdata->stress.ctx[2].rx_cnt);
			LOG_ERR("tx_cnt0:%d(%02x) tx_cnt1:%d(%02x) tx_cnt2:%d(%02x)",
					tdata->stress.ctx[0].cnt,
					tdata->stress.ctx[0].cnt,
					tdata->stress.ctx[1].cnt,
					tdata->stress.ctx[1].cnt,
					tdata->stress.ctx[2].cnt,
					tdata->stress.ctx[2].cnt);
			zassert_true(false,
				"Unexpected byte at %d, got %02x exp:%02x",
				i, pkt_data[i], (uint8_t)(rx_cnt + i));
		}
	}
}
...
static void *setup(void)
{
	...
	TC_PRINT("Testing %s IPC service\n",
			IS_ENABLED(CONFIG_IPC_SERVICE_BACKEND_MPSC) ? "mpsc" :
...
ZTEST_SUITE(_ipc_service_benchmark, NULL, setup, NULL, after, NULL);
