...
struct uart_driver_api {
	...
	const struct uart_driver_irq_api * irq_api;
				 ...
				 int size)
{
	...
	if (api->irq_api) {
		return api->irq_api->fifo_fill(dev, tx_data, size);
	}
...
static inline int uart_fifo_read(struct device *dev, u8_t *rx_data,
				 const int size)
{
	const struct uart_driver_irq_api *api =
		((const struct uart_driver_api *)dev->driver_api)->irq_api;
...
static inline void _impl_uart_irq_tx_disable(struct device *dev)
{
	...
	if (api->irq_api) {
		api->irq_api->irq_tx_disable(dev);
	}
}
...
static inline int uart_irq_tx_ready(struct device *dev)
{
	...
	if (api->irq_api) {
		return api->irq_api->irq_tx_ready(dev);
	}
...
static inline void _impl_uart_irq_rx_disable(struct device *dev)
{
	...
	if (api->irq_api) {
		api->irq_api->irq_rx_disable(dev);
	}
}
...
static inline int uart_irq_tx_complete(struct device *dev)
{
	...
	if (api->irq_api && api->irq_api->irq_tx_complete) {
		return api->irq_api->irq_tx_complete(dev);
	}
...
static inline int uart_irq_rx_ready(struct device *dev)
{
	...
	if (api->irq_api) {
		return api->irq_api->irq_rx_ready(dev);
	}
...
static inline void _impl_uart_irq_err_enable(struct device *dev)
{
	...
	if (api->irq_api) {
		api->irq_api->irq_err_enable(dev);
	}
}
...
static inline void _impl_uart_irq_err_disable(struct device *dev)
{
	...
	if (api->irq_api) {
		api->irq_api->irq_err_disable(dev);
	}
}
...
static inline int _impl_uart_irq_is_pending(struct device *dev)
{
	...
	if (api->irq_api)	{
		return api->irq_api->irq_is_pending(dev);
	}
...
static inline int _impl_uart_irq_update(struct device *dev)
{
	...
	if (api->irq_api) {
		return api->irq_api->irq_update(dev);
	}
					...
					void *user_data)
{
	...
	if (api->irq_api && api->irq_api->irq_callback_set) {
		api->irq_api->irq_callback_set(dev, cb, user_data);
	}
}
