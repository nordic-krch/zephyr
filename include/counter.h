 ...
 * otherwise counter is reset. If counter is counting up and free running it is
 * possible that new top value is smaller than current counter value. In that
 * case counter may go out of bounds. @ref counter_set_top_value() will return
 * error -ETIME if such situation is detected.
 */
 ...
 *		  is smaller than current counter value (counting up).
 */
