 ...
 *				messages length).
 * @return struct icmsg_buf*	Pointer to the created buffer. The pointer
 ...
 * @return int	Number of bytes written, negative error code on fail.
 *		-EINVAL, if len is bigger than the icmsg buffer can fit at all
 *			or the len == 0.
 *		-ENOMEM, if the buffer is occupied and the reader did not yet
 *			read the data.
 */
 ...
 * @return int		Bytes read, negative error code on fail.
 *			Bytes to be read, if buf == NULL.
 *			-EINVAL, if message can not fit in provided buf.
