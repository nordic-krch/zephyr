 ...
 * overflow mode is selected. In that mode, oldest entry are dropped (user is
 ...
 */
#define MPSC_PBUF_MODE_OVERFLOW BIT(1)
 ...
 */
typedef uint32_t (*mpsc_pbuf_get_len)(union mpsc_pbuf_generic *packet);
 ...
 *
 * If a buffer is configured to overflow mode then if there is no space to
 ...
 * @param buffer Buffer.
 *
 * @param pointer to a packet or null if no pending packets.
 */
