 ...
 */
static inline u8_t log_link_get_domain_count(const struct log_link *link)
{
	__ASSERT_NO_MSG(link);
 ...
 * @return Source count.
 */
static inline u16_t log_link_get_source_count(const struct log_link *link,
					      u8_t domain_id)
{
	__ASSERT_NO_MSG(link);
 ...
 */
static inline int log_link_count_get(void)
{
