...
/* Messages are aligned to 2 words */
#define Z_LOG_MSG2_ALIGNMENT CBPRINTF_PACKAGE_ALIGNMENT

#define Z_LOG_MSG2_ALLOCA(ptr, len) \
	long long *_ll_ptr = alloca(len); \
	long double *_ld_ptr = alloca(len); \
	ptr = (sizeof(long double) == Z_LOG_MSG2_ALIGNMENT) ? \
			(void *)_ll_ptr : (void *)_ld_ptr;\
...
#define Z_LOG_MSG2_SYNC(_domain_id, _source, _level, _data, _dlen, ...) do { \
	...
	Z_LOG_MSG2_ALLOCA(_msg, Z_LOG_MSG2_LEN(_plen, _dlen)); \
	if (_plen) {\
...
do { \
	...
	struct log_msg2 *_msg; \
	Z_LOG_MSG2_ALLOCA(_msg, Z_LOG_MSG2_LEN(_plen, 0)); \
	if (_plen) { \
...
#else
/* Some compilers (seen on arm) are so dump when optimization is off that
 * huge space on the stack is allocated for code that is not even compiled in.
 * Because of that, stack overflows. Adding dedicated macro when optimization
 * is off, which performs runtime message creation always (we don't care about
 * performance now).
 */
#define Z_LOG_MSG2_CREATE2(_try_0cpy, _mode,  _cstr_cnt, _domain_id, _source,\
			   _level, _data, _dlen, ...) \
do { \
	z_log_msg2_runtime_create(_domain_id, (void *)_source, _level, \
				  (uint8_t *)_data, _dlen, __VA_ARGS__);\
	_mode = Z_LOG_MSG2_MODE_RUNTIME; \
} while (0)
 ...
 * @oaram data Data.
 */
void z_log_msg2_static_create(const void *source,
 ...
 */
void z_log_msg2_runtime_vcreate(uint8_t domain_id, const void *source,
