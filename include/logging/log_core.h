...
#define Z_LOG_MINIMAL_TO_PRINTK(_level, fmt, ...) do { \
	...
	do {								       \
		if (Z_LOG_CONST_LEVEL_CHECK(_level)) {			       \
			...
			if (IS_ENABLED(CONFIG_LOG_MINIMAL)) {		       \
				Z_LOG_MINIMAL_TO_PRINTK(_level, __VA_ARGS__);  \
	...
	do {								       \
		if (Z_LOG_CONST_LEVEL_CHECK(_level)) {			       \
			bool is_user_context = _is_user_context();	       \
									       \
			if (IS_ENABLED(CONFIG_LOG_MINIMAL)) {		       \
				Z_LOG_MINIMAL_TO_PRINTK(_level, "%s", _str);   \
	...
	do {								       \
		if (Z_LOG_CONST_LEVEL_CHECK(_level)) {			       \
			bool is_user_context = _is_user_context();	       \
									       \
			if (IS_ENABLED(CONFIG_LOG_MINIMAL)) {		       \
				z_log_minimal_vprintk(_str, _valist);	       \
			} else if (LOG_CHECK_CTX_LVL_FILTER(is_user_context, \
					_level, _filter)) {  \
				struct log_msg_ids src_level = {	       \
					.level = _level,		       \
