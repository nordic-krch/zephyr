...
BUILD_ASSERT(CB_BUF_SIZE <= UINT8_MAX);
...
static void mbox_callback_process(struct k_work *item)
{
	...
	if (state == ICMSG_STATE_READY) {
		...
		/* Reading with NULL buffer to know if there are data in the buffer to be read. */
...
static int backend_init(const struct device *instance)
{
	...
	__ASSERT_NO_MSG(conf->rx_shm_size > sizeof(struct icmsg_buf));
	__ASSERT_NO_MSG(conf->tx_shm_size > sizeof(struct icmsg_buf));

	dev_data->rx_ib = icmsg_buf_init((void *)conf->rx_shm_addr, conf->rx_shm_size);
