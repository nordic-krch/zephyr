...
{
	int err;
	struct log_backend_remote *backend_remote = backend->cb->ctx;
	...
	uint8_t __aligned(sizeof(void *)) buf[msg_len + sizeof(void *)];
	...
	struct log_msg2 *out_log_msg = (struct log_msg2 *)out_msg->data.log_msg.data;

	/* Set ipc message id. */
	out_msg->id = Z_LOG_MULTIDOMAIN_ID_MSG;
	out_msg->status = Z_LOG_MULTIDOMAIN_STATUS_OK;
	/* Copy log message header. */
	memcpy(&out_log_msg->hdr, &msg->log.hdr, sizeof(struct log_msg2_hdr));
...
static void get_name_response(struct log_backend_remote *backend_remote,
				   ...
				   const void *data, size_t len)
{
	...
	case Z_LOG_MULTIDOMAIN_ID_GET_SOURCE_CNT:
		outmsg.data.source_cnt.count =
			log_sources_count(msg->data.source_cnt.domain_id);
		break;
...
const struct log_backend_api log_backend_remote_api = {
	.process = process,
	.panic = panic,
	.dropped = dropped,
	.init = init
