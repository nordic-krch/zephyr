				...
				const void *data, size_t len)
{
	...
	{
		uint32_t slen = MIN(len - 1, *link_remote->dst.name.len - 1);
...
static int getter_msg_process(struct log_link_remote *link_remote,
...
static int link_remote_get_domain_count(struct log_link_remote *link_remote,
					uint16_t *cnt)
{
	int err;
...
static int link_remote_get_source_count(struct log_link_remote *link_remote,
					      uint32_t domain_id,
					      uint16_t *cnt)
{
	int err;
...
{
	struct log_link_remote *link_remote = link->ctx;
...
struct log_link_api log_link_remote_api = {
