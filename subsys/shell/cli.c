...
#include <ext/lib/fnmatch/fnmatch.h>
    ...
    #if CONFIG_SHELL_HISTORY_ELEMENT_SIZE * CONFIG_SHELL_HISTORY_ELEMENT_COUNT == 0
...
#define SHELL_HELP_STATISTICS_SHOW    "Get CLI statistics for the Logger module."
#define SHELL_HELP_STATISTICS_RESET   "Reset CLI statistics for the Logger module."
#define SHELL_HELP_RESIZE             "Console gets terminal screen size or assumes 80 in case " \
                                        "the readout fails. It must be executed after each terminal " \
                                        "width change to ensure correct text display."
#define SHELL_HELP_RESIZE_DEFAULT     "Assume 80 chars screen width and send this setting "     \
                                        "to the terminal."
#define SHELL_HELP_HISTORY            "Command history."
#define SHELL_HELP_ECHO               "Toggle CLI echo."
#define SHELL_HELP_ECHO_ON            "Enable CLI echo."
#define SHELL_HELP_ECHO_OFF           "Disable CLI echo. Arrows and buttons: Backspace, "       \
...
#define SHELL_MSG_TAB_OVERFLOWED      "Tab function: commands counter overflowed.\r\n"

/*lint -save -esym(526,cli_command*) -esym(526,cli_sorted_cmd_ptrs*)*/
NRF_SECTION_DEF(cli_command, shell_cmd_entry_t);
#define CLI_DATA_SECTION_ITEM_GET(i) NRF_SECTION_ITEM_GET(cli_command, shell_cmd_entry_t, (i))
#define CLI_DATA_SECTION_ITEM_COUNT  NRF_SECTION_ITEM_COUNT(cli_command, shell_cmd_entry_t)
...
{
    shell->p_ctx->internal.flag.show_help = 0;
}

static inline void cli_flag_echo_set(shell_t const * shell)
{
    shell->p_ctx->internal.flag.echo = 1;
}

static inline void cli_flag_echo_clear(shell_t const * shell)
{
    shell->p_ctx->internal.flag.echo = 0;
}

static inline bool cli_flag_echo_is_set(shell_t const * shell)
{
    return shell->p_ctx->internal.flag.echo == 1 ? true : false;
}

static inline bool cli_flag_processing_is_set(shell_t const * shell)
{
    return shell->p_ctx->internal.flag.processing == 1 ? true : false;
}

static inline void receive_state_change(shell_t const * shell, shell_receive_t state)
{
    shell->p_ctx->receive_state = state;
}
...
{
    shell->p_ctx->cmd_buff[0] = '\0';  /* clear command buffer */
    shell->p_ctx->cmd_buff_pos = 0;
    shell->p_ctx->cmd_buff_len = 0;
}

/* Function returns true if cursor is at beginning of an empty line. */
static inline bool cursor_in_empty_line(shell_t const * shell)
{
    return ( (shell->p_ctx->cmd_buff_pos + cli_strlen(shell->p_name)) %
              shell->p_ctx->vt100_ctx.cons.terminal_wid == 0);
}
...
{
    return ((shell->p_ctx->cmd_buff_len + cli_strlen(shell->p_name)) %
            shell->p_ctx->vt100_ctx.cons.terminal_wid == 0);
}
...
static void cli_write(shell_t const * shell,
                      void const *      p_data,
                      size_t            length,
                      size_t *          p_cnt)
{
    ...
    while (length)
    {
        ...
        offset += cnt;
        length -= cnt;
        if (cnt == 0 && (shell->p_ctx->state != SHELL_STATE_PANIC_MODE_ACTIVE))
        {
#if NRF_MODULE_ENABLED(SHELL_USES_TASK_MANAGER)
            (void)task_events_wait(SHELL_TRANSPORT_TX_RDY_TASK_EVT);
#else
            while (shell->p_ctx->internal.flag.tx_rdy == 0)
            {
                ;
            }
            shell->p_ctx->internal.flag.tx_rdy = 0;
#endif
        }
    }
...
static void cli_read(shell_t const * shell,
                     void *            p_data,
                     size_t            length,
                     size_t *          p_cnt)
{
    ...
    ret_code_t ret = shell->p_iface->p_api->read(shell->p_iface, p_data, length, p_cnt);
    UNUSED_VARIABLE(ret);
}
 ...
 */
static void cmd_get(shell_cmd_entry_t const *     p_command,
                    size_t                          lvl,
                    size_t                          idx,
 ...
 * =>                   cur_y = 2, cur_y_end = 3
 */
static shell_multiline_cons_t const * multiline_console_data_check(shell_t const * shell)
{
    shell_ctx_t * p_ctx = shell->p_ctx;
    shell_multiline_cons_t * p_cons = &shell->p_ctx->vt100_ctx.cons;

    p_cons->name_len = cli_strlen(shell->p_name);
...
{
    if (shell->p_ctx->cmd_buff_pos >= shell->p_ctx->cmd_buff_len)
    {
        return; /* incrementation not possible */
    }

    shell_multiline_cons_t const * p_cons = multiline_console_data_check(shell);
    shell_cmd_len_t cur_y = p_cons->cur_y;
    ++shell->p_ctx->cmd_buff_pos;
    p_cons = multiline_console_data_check(shell);
...
static void cursor_position_synchronize(shell_t const * shell)
{
    ...
    if (last_line)
    {
        cursor_left_move(shell, p_cons->cur_x_end - p_cons->cur_x);
    }
    else
    {
        cursor_up_move(shell, p_cons->cur_y_end - p_cons->cur_y);
        if (p_cons->cur_x > p_cons->cur_x_end)
        {
            cursor_right_move(shell, p_cons->cur_x - p_cons->cur_x_end);
        }
        else
        {
            cursor_left_move(shell, p_cons->cur_x_end - p_cons->cur_x);
...
static void cli_cmd_word_remove(shell_t const * shell)
{
    ...
    if ((shell->p_ctx->cmd_buff_len == 0) || (shell->p_ctx->cmd_buff_pos == 0))
    {
        return;
    }

    /* start at the current position */
    new_pos = shell->p_ctx->cmd_buff_pos;
    chars_to_delete = 0;

    /* look back for all spaces then for non-spaces */
    while ((new_pos >= 1) && (shell->p_ctx->cmd_buff[new_pos - 1] == ' '))
    {
        ++chars_to_delete;
        --new_pos;
    }
    ...
    row_span = cli_row_span_with_buffer_offsets_get(shell, shell->p_ctx->cmd_buff_pos, new_pos);
    col_span = cli_column_span_with_buffer_offsets_get(shell, shell->p_ctx->cmd_buff_pos, new_pos);

    /* manage the buffer */
    memmove(&shell->p_ctx->cmd_buff[new_pos],
            &shell->p_ctx->cmd_buff[new_pos + chars_to_delete],
            shell->p_ctx->cmd_buff_len - chars_to_delete);
    shell->p_ctx->cmd_buff_len -= chars_to_delete;
    shell->p_ctx->cmd_buff_pos = new_pos;
    shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_len] = '\0';
    ...
    shell_fprintf(shell,
            SHELL_NORMAL,
            "%s",
            &shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos]);
    cli_clear_eos(shell);
...
static void cursor_home_position_move(shell_t const * shell)
{
    ...
    if ((p_cons->cur_x == p_cons->name_len + SHELL_INITIAL_CURS_POS) &&
        (p_cons->cur_y == SHELL_INITIAL_CURS_POS))
    {
        return; /* nothing to handle because cursor is in start position */
    }

    if (p_cons->cur_y > SHELL_INITIAL_CURS_POS)
    {
        cursor_up_move(shell, p_cons->cur_y - SHELL_INITIAL_CURS_POS);
    }

    if (p_cons->cur_x > p_cons->name_len)
    {
        cursor_left_move(shell, p_cons->cur_x - SHELL_INITIAL_CURS_POS - p_cons->name_len);
    }
    else
    {
        cursor_right_move(shell, p_cons->name_len + SHELL_INITIAL_CURS_POS - p_cons->cur_x);
    }
...
static void cursor_end_position_move(shell_t const * shell)
{
    ...
    {
        cursor_down_move(shell, p_cons->cur_y_end - p_cons->cur_y);
    }

    if (p_cons->cur_x > p_cons->cur_x_end)
    {
        cursor_left_move(shell, p_cons->cur_x - p_cons->cur_x_end);
    }
    else
    {
        cursor_right_move(shell, p_cons->cur_x_end - p_cons->cur_x);
    }
    /* align data buffer pointer with cursor position */
    shell->p_ctx->cmd_buff_pos = shell->p_ctx->cmd_buff_len;
}
...
static ret_code_t cursor_position_get(shell_t const * shell)
{
    ...
    /* clear temp buffer */
    memset(shell->p_ctx->temp_buff, 0, sizeof(shell->p_ctx->temp_buff));
    ...
    for (uint16_t i = 0; i < 1000; i++)
    {
        do
        {
            ...
            if ((c != SHELL_VT100_ASCII_ESC) &&
                (shell->p_ctx->temp_buff[0] != SHELL_VT100_ASCII_ESC))
            {
                continue;
            }
            ...
            {
                shell->p_ctx->temp_buff[buff_idx] = '\0';
                if (shell->p_ctx->temp_buff[1] != '[')
                {
                    shell->p_ctx->temp_buff[0] = 0;
                    return NRF_ERROR_INVALID_DATA;
                }
                buff_idx = 2;  /* index start position in the buffer where 'y' is stored */
                while (shell->p_ctx->temp_buff[buff_idx] != ';')
                {
                    y = y * 10 + (shell->p_ctx->temp_buff[buff_idx++] - '0');
                ...
                while (shell->p_ctx->temp_buff[buff_idx] != '\0')
                {
                    x = x * 10 + (shell->p_ctx->temp_buff[buff_idx++] - '0');
                    if (buff_idx >= SHELL_CMD_BUFF_SIZE)
                    {
                        return NRF_ERROR_DATA_SIZE;
                    }
                ...
                {
                    shell->p_ctx->vt100_ctx.cons.cur_x = SHELL_MAX_TERMINAL_SIZE;
                }
                else
                {
                    shell->p_ctx->vt100_ctx.cons.cur_x = (shell_cmd_len_t)x;
                }
                /* vertical cursor position */
                if (y > SHELL_MAX_TERMINAL_SIZE)
                {
                    shell->p_ctx->vt100_ctx.cons.cur_y = SHELL_MAX_TERMINAL_SIZE;
                }
                else
                {
                    shell->p_ctx->vt100_ctx.cons.cur_y = (shell_cmd_len_t)y;
                }
                shell->p_ctx->temp_buff[0] = 0;
                return NRF_SUCCESS;
            }
            ...
            {
                shell->p_ctx->temp_buff[buff_idx] = c;
            }
                                    ...
                                    shell_cmd_len_t * p_height)
{
    ...
    {
        x = shell->p_ctx->vt100_ctx.cons.cur_x;
        y = shell->p_ctx->vt100_ctx.cons.cur_y;
    ...
    if (cursor_position_get(shell) == NRF_SUCCESS)
    {
        *p_length = shell->p_ctx->vt100_ctx.cons.cur_x;
        *p_height = shell->p_ctx->vt100_ctx.cons.cur_y;
...
static void vt100_color_set(shell_t const * shell, shell_vt100_color_t color)
{
    ...
    {
        if (shell->p_ctx->vt100_ctx.col.col == color)
        {
            return;
        }
        ...
        shell->p_ctx->vt100_ctx.col.col = color;
...
static void vt100_bgcolor_set(shell_t const * shell, shell_vt100_color_t bgcolor)
{
    ...
    {
        if (shell->p_ctx->vt100_ctx.col.bgcol == bgcolor)
        {
            return;
        }
        ...
        shell->p_ctx->vt100_ctx.col.bgcol = bgcolor;
        nrf_fprintf(shell->p_fprintf_ctx, "%s", cmd);
    }
}
...
{
    memcpy(p_color, &shell->p_ctx->vt100_ctx.col, sizeof(shell_vt100_colors_t));
}
...
static void left_arrow_handle(shell_t const * shell)
{
    ...
    if (p_cons->cur_x == SHELL_INITIAL_CURS_POS)
    {   /* go to previous line */
        cursor_up_move(shell, 1);
        cursor_right_move(shell, p_cons->terminal_wid);
        --shell->p_ctx->cmd_buff_pos;
    }
    else
    {
...
static void right_arrow_handle(shell_t const * shell)
{
    ...
    if (p_cons->cur_x == p_cons->terminal_wid) /* go to next line */
    {
        ...
        cursor_left_move(shell, p_cons->terminal_wid);
        ++shell->p_ctx->cmd_buff_pos;
    }
    else
    {
...
{
    if (shell->p_ctx->cmd_buff_len >= (SHELL_CMD_BUFF_SIZE - 1))
    {
        return;
    }

    shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos++] = data;
    shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos] = '\0';
    ++shell->p_ctx->cmd_buff_len;
}

static void char_insert(shell_t const * shell, char data)
{
    shell_cmd_len_t diff;
    bool ins_mode = (bool)shell->p_ctx->internal.flag.insert_mode;

    diff = shell->p_ctx->cmd_buff_len - shell->p_ctx->cmd_buff_pos;
    ...
    if (!ins_mode)
    {
        ...
        if (diff > 0)
        {
            memmove(&shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos + 1],
                    &shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos],
                    diff);
        }
    }
    ...
    if (full_line_cmd(shell))
    {
        ...
        cursor_down_move(shell, p_cons->cur_y_end - p_cons->cur_y - 1);
        cursor_next_line_move(shell);
        cursor_up_move(shell, p_cons->cur_y_end - p_cons->cur_y);
        cursor_right_move(shell, p_cons->cur_x - 1);
        return;
    }
}
...
static void char_delete(shell_t const * shell)
{
    ...
    memmove(&shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos],
            &shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos + 1],
            diff);
...
static inline void history_mode_exit(shell_t const * shell)
{
    shell->p_ctx->p_cmd_list_element = NULL;
}

static void history_handle(shell_t const * shell, bool up)
{
    ...
    shell_cmd_len_t current_cmd_len;
    bool skip = false;

    if (!up) /* button down */
    {
        if (shell->p_ctx->p_cmd_list_element == NULL)
        {
            return;
        }
        cursor_home_position_move(shell);

        nrf_memobj_read(shell->p_ctx->p_cmd_list_element,
                        &header,
                        SHELL_HISTORY_HEADER_SIZE,
                        0);

        shell->p_ctx->p_cmd_list_element = header.p_next;
        current_cmd_len = shell->p_ctx->cmd_buff_len;

        if (shell->p_ctx->p_cmd_list_element == NULL)
        {
            if (cli_strlen(shell->p_ctx->temp_buff) > 0)
            {
                strcpy(shell->p_ctx->cmd_buff, shell->p_ctx->temp_buff);
            }
            else
            {
                shell->p_ctx->cmd_buff[0] = '\0';
            }
            header.cmd_len = cli_strlen(shell->p_ctx->cmd_buff);
            skip = true;
        }
    }
    else /* button up */
    {
        if ((shell->p_ctx->p_cmd_list_element == shell->p_ctx->p_cmd_list_tail) ||
            (shell->p_ctx->p_cmd_list_head == NULL))
        {
            /* Nothing to display. */
            return;
        }
        ...
        if (shell->p_ctx->p_cmd_list_element == NULL)
        {
            current_cmd_len = cli_strlen(shell->p_ctx->cmd_buff);

            shell->p_ctx->p_cmd_list_element = shell->p_ctx->p_cmd_list_head;
            /* Save the currently entered and not executed command. */
            if (current_cmd_len > 0)
            {
                strcpy(shell->p_ctx->temp_buff, shell->p_ctx->cmd_buff);
            }
            else
            {
                shell->p_ctx->temp_buff[0] = '\0';
        ...
        else
        {
            ...
            shell->p_ctx->p_cmd_list_element = header.p_prev;
        }
    }
    if (!skip)
    {
        ...
        nrf_memobj_read(shell->p_ctx->p_cmd_list_element,
                        shell->p_ctx->cmd_buff,
                        header.cmd_len + 1, /* +1 for '\0' */
                        SHELL_HISTORY_HEADER_SIZE);
    }

    shell->p_ctx->cmd_buff_pos = header.cmd_len;
    shell->p_ctx->cmd_buff_len = header.cmd_len;
    ...
    shell_fprintf(shell, SHELL_NORMAL, "%s", shell->p_ctx->cmd_buff);
    if (cursor_in_empty_line(shell) || full_line_cmd(shell))
    {
        cursor_next_line_move(shell);
    }
...
static void history_list_element_add(shell_t const * shell, nrf_memobj_t * p_memobj)
{
    ASSERT(p_memobj != NULL);

    shell_memobj_header_t header;

    if (shell->p_ctx->p_cmd_list_head == NULL)
    {
        shell->p_ctx->p_cmd_list_head = p_memobj;
        shell->p_ctx->p_cmd_list_tail = p_memobj;
        header.p_prev = NULL;
        header.p_next = NULL;
        header.cmd_len = shell->p_ctx->cmd_buff_len;
    }
    else
    {
        nrf_memobj_read(shell->p_ctx->p_cmd_list_head,
                        &header,
                        SHELL_HISTORY_HEADER_SIZE,
                        0);
        ...
        nrf_memobj_write(shell->p_ctx->p_cmd_list_head,
                         &header,
                         SHELL_HISTORY_HEADER_SIZE,
                         0);

        header.p_next = NULL;
        header.p_prev = shell->p_ctx->p_cmd_list_head;
        header.cmd_len = shell->p_ctx->cmd_buff_len;
    ...
    nrf_memobj_write(p_memobj,
                     shell->p_ctx->cmd_buff,
                     shell->p_ctx->cmd_buff_len + 1, /* +1 for '\0' */
                     SHELL_HISTORY_HEADER_SIZE);
}

static void history_list_element_oldest_remove(shell_t const * shell)
{
    if (shell->p_ctx->p_cmd_list_tail == NULL)
    {
        return; // nothing to do
    }

    shell_memobj_header_t header;
    nrf_memobj_t * p_memobj = shell->p_ctx->p_cmd_list_tail;
    ...
    shell->p_ctx->p_cmd_list_tail = header.p_next;
    memset(&header, 0, sizeof(shell_memobj_header_t));
    nrf_memobj_write(p_memobj, &header, SHELL_HISTORY_HEADER_SIZE, 0);
    ...
    nrf_memobj_read(shell->p_ctx->p_cmd_list_tail,
                    &header,
                    SHELL_HISTORY_HEADER_SIZE,
                    0);

    header.p_prev = NULL;
    nrf_memobj_write(shell->p_ctx->p_cmd_list_tail, &header, SHELL_HISTORY_HEADER_SIZE, 0);
}

static void history_list_free_memory(shell_t const * shell)
{
    while (shell->p_ctx->p_cmd_list_tail != NULL)
    {
        history_list_element_oldest_remove(shell);
    }
}

static void history_save(shell_t const * shell)
{
    shell_cmd_len_t cmd_new_len = cli_strlen(shell->p_ctx->cmd_buff);
    ...
    if (shell->p_ctx->p_cmd_list_head != NULL)
    {
        ...
        {
            nrf_memobj_read(shell->p_ctx->p_cmd_list_head,
                            shell->p_ctx->temp_buff,
                            header.cmd_len + 1, /* +1 for '\0' */
                            SHELL_HISTORY_HEADER_SIZE);

            if (strcmp(shell->p_ctx->cmd_buff, shell->p_ctx->temp_buff) == 0)
            {
    ...
    for (size_t idx = 0; idx < SHELL_HISTORY_ELEMENT_COUNT; idx++)
    {
        nrf_memobj_t * p_memobj;

        p_memobj = nrf_memobj_alloc(shell->p_cmd_hist_mempool,
                                    ((size_t)SHELL_HISTORY_HEADER_SIZE + cmd_new_len + 1));
        if (p_memobj != NULL)
        {
            history_list_element_add(shell, p_memobj);
            return;
        }
        else
        {
...
static shell_cmd_len_t str_similarity_check(char const * str_a, char const * str_b)
{
...
static void completion_insert(shell_t const * shell,
                              char const *      p_compl,
                              shell_cmd_len_t compl_len)
{
    ASSERT (p_compl);

    shell_cmd_len_t diff = shell->p_ctx->cmd_buff_len - shell->p_ctx->cmd_buff_pos;

    if ((shell->p_ctx->cmd_buff_len + compl_len > SHELL_CMD_BUFF_SIZE - 1) ||
        (compl_len == 0))
    {
        return;
    }
    ...
    memmove(&shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos + compl_len],
            &shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos],
            diff + 1); /* + 1 for '\0' */
    ...
    shell->p_ctx->cmd_buff_len = cli_strlen(shell->p_ctx->cmd_buff);
    shell_fprintf(shell,
                    SHELL_NORMAL,
                    "%s",
                    &shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos]);
    shell->p_ctx->cmd_buff_pos += compl_len;
...
static void option_print(shell_t const * shell,
                         char const *      p_option,
                         shell_cmd_len_t longest_option)
{
    ...
    if (p_option == NULL)
    {
        shell->p_ctx->vt100_ctx.printed_cmd = 0;
        return;
    }
    longest_option += cli_strlen(tab);

    shell_cmd_len_t columns =
        (shell->p_ctx->vt100_ctx.cons.terminal_wid - cli_strlen(tab)) / longest_option;
    shell_cmd_len_t diff = longest_option - cli_strlen(p_option);

    if (shell->p_ctx->vt100_ctx.printed_cmd++ % columns == 0)
    {
...
static void cli_tab_handle(shell_t const * shell)
{
    ...
    shell_cmd_len_t compl_len = (SHELL_CMD_BUFF_SIZE - 1) - shell->p_ctx->cmd_buff_len;
    ...
    memcpy(shell->p_ctx->temp_buff,
           shell->p_ctx->cmd_buff,
           shell->p_ctx->cmd_buff_pos);

    shell->p_ctx->temp_buff[shell->p_ctx->cmd_buff_pos] = '\0';

    /* Check if the current cursor position points to the 'space' character. */
    bool space = isspace((int)shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos - 1]);
    ...
    do
    {
            ...
            (cmd_lvl >= argc - 1 + space))
        {
            ...
            while (1)
            {
                ...
                {
                    shell_cmd_len_t len = cli_strlen(p_st_cmd->p_syntax);
                    if (len > cmd_longest)
                    {
                        cmd_longest = len;
                    }
                ...
                {
                    shell_fprintf(shell, SHELL_WARNING, SHELL_MSG_TAB_OVERFLOWED);
                    break;
                }
            }
        }
    ...
    if (cmd_first == cmd_last) /* only one match found */
    {
        ...
        if (!isspace((int)shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_pos]))
        {
            if (shell->p_ctx->internal.flag.insert_mode)
            {
                shell->p_ctx->internal.flag.insert_mode = 0;
                char_insert(shell, ' ');
                shell->p_ctx->internal.flag.insert_mode = 1;
            }
            else
            {
                char_insert(shell, ' ');
...
{
    return (uint8_t)data > SHELL_ASCII_MAX_CHAR ? NRF_ERROR_INVALID_DATA : NRF_SUCCESS;
}
...
static void cli_state_collect(shell_t const * shell)
{
    ...
    while (1)
    {
        ...
        switch (shell->p_ctx->receive_state)
        {
            case SHELL_RECEIVE_DEFAULT:
                if (data == shell->newline_char)
                {
                    if (shell->p_ctx->cmd_buff_len == 0)
                    {
                        history_mode_exit(shell);
                        cursor_next_line_move(shell);
                    }
                ...
                switch (data)
                {
                    ...
                    default:
                        if (isprint((int)data))
                        {
                            ...
                            {
                                char_insert_echo_off(shell, data);
                            }
                        }
                        break;
                }
            ...
            case SHELL_RECEIVE_ESC:
                if (data == '[')
                {
                    receive_state_change(shell, SHELL_RECEIVE_ESC_SEQ);
                }
                else
                {
            ...
            case SHELL_RECEIVE_ESC_SEQ:
                ...
                switch (data)
                {
                    ...
                    case '4': /* END Button in ESC[n~ mode */
                        receive_state_change(shell, SHELL_RECEIVE_TILDE_EXP);
                        /* fall through */
...
static void cmd_trim(shell_t const * shell)
{
    ...
    if (shell->p_ctx->cmd_buff[0] == '\0') /* no command in the buffer */
    {
        return;
    }

    /* Counting white characters starting from beginning of the command. */
    while (isspace((int)shell->p_ctx->cmd_buff[i++]))
    {
        if (i == 0)
        {
            shell->p_ctx->cmd_buff[0] = '\0';
    ...
    if (--i > 0)
    {
        memmove(shell->p_ctx->cmd_buff,
                shell->p_ctx->cmd_buff + i,
                (shell->p_ctx->cmd_buff_len + 1) - i); /* +1 for '\0' */
        shell->p_ctx->cmd_buff_len = shell->p_ctx->cmd_buff_len - i;
        shell->p_ctx->cmd_buff_pos = shell->p_ctx->cmd_buff_len;
    }

    /* Counting white characters starting from end of command. */
    char * p_end = &shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_len - 1];
    i = 0;
    ...
    /* Removing counted white characters. */
    if (p_end != &shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_len - 1])
    {
        shell->p_ctx->cmd_buff[shell->p_ctx->cmd_buff_len - i] = '\0';
        shell->p_ctx->cmd_buff_len = shell->p_ctx->cmd_buff_len - i;
        shell->p_ctx->cmd_buff_pos = shell->p_ctx->cmd_buff_len;
    }
}
                                      ...
                                      char const *      p_pattern)
{
    ...
    /* +1 for space */
    if (((size_t)shell->p_ctx->cmd_tmp_buff_len + cmd_len + 1) > SHELL_CMD_BUFF_SIZE)
    {
        shell_fprintf(shell,
                        SHELL_WARNING,
    ...
    p_cmd_source_addr = strstr(shell->p_ctx->temp_buff, p_pattern);
    ...
    shell->p_ctx->cmd_tmp_buff_len += cmd_len + 1; // + 1 for space
...
{
    size_t shift;
    char * p_pattern_addr = strstr(shell->p_ctx->temp_buff, p_pattern);
    ...
    if (p_pattern_addr > shell->p_ctx->temp_buff)
    {
        if (*(p_pattern_addr - 1) == ' ')
        {
    ...
    shell->p_ctx->cmd_tmp_buff_len -= pattern_len;
...
*/
    ...
    /* Function spaces_trim must be used instead of make_argv. At this point it is important to keep
       ...
       function. */
    spaces_trim(shell->p_ctx->temp_buff);
    shell->p_ctx->cmd_tmp_buff_len = cli_strlen(shell->p_ctx->temp_buff) + 1;  // +1 for EOS
#endif
    ...
    {
        shell->p_ctx->p_current_stcmd = p_cmd->u.p_static;
    }
    ...
    if (commands_expanded > 0)
    {
        ...
        memcpy(shell->p_ctx->cmd_buff,
               shell->p_ctx->temp_buff,
               shell->p_ctx->cmd_tmp_buff_len);
        shell->p_ctx->cmd_buff_len = shell->p_ctx->cmd_tmp_buff_len;
    ...
    if (p_cmd_low_level_entry != NULL)
    {
        ...
        shell->p_ctx->p_current_stcmd = p_static_entry;

        shell->p_ctx->p_current_stcmd->handler(shell,
...
static void cli_transport_evt_handler(shell_transport_evt_t evt_type, void * p_context)
{
    shell_t * shell = (shell_t *)p_context;
    ASSERT(shell);
#if NRF_MODULE_ENABLED(SHELL_USES_TASK_MANAGER)
    task_events_set(shell->p_ctx->task_id, evt_type == SHELL_TRANSPORT_EVT_RX_RDY ?
    ...
    {
        /* wr done evt */
        shell->p_ctx->internal.flag.tx_rdy = 1;
    }
#endif
}
...
{
    ASSERT(shell);
    ASSERT(shell->p_ctx && shell->p_iface && shell->p_name);
    ASSERT((shell->newline_char == '\n') || (shell->newline_char == '\r'));

#if defined(SHELL_LOG_BACKEND) && SHELL_LOG_BACKEND
    ((shell_log_backend_t *)shell->p_log_backend->p_ctx)->shell = shell;
#endif
    ret_code_t ret = shell->p_iface->p_api->init(shell->p_iface,
                                                 p_config,
    ...
    shell->p_ctx->p_cmd_list_head = NULL;
    shell->p_ctx->p_cmd_list_tail = NULL;
#endif

    memset(shell->p_ctx, 0, sizeof(shell_ctx_t));
    shell->p_ctx->internal.flag.tx_rdy = 1;

#if NRF_MODULE_ENABLED(SHELL_VT100_COLORS)
    shell->p_ctx->internal.flag.use_colors = use_colors;
#endif
    shell->p_ctx->internal.flag.echo = SHELL_ECHO_STATUS;
    shell->p_ctx->state = SHELL_STATE_INITIALIZED;
    shell->p_ctx->vt100_ctx.cons.terminal_wid = SHELL_DEFAULT_TERMINAL_WIDTH;
    shell->p_ctx->vt100_ctx.cons.terminal_hei = SHELL_DEFAULT_TERMINAL_HEIGHT;
...
ret_code_t shell_init(shell_t const *  shell,
                        void const *       p_config,
                        bool               use_colors,
                        bool               log_backend,
...
{
#if NRF_MODULE_ENABLED(SHELL_USES_TASK_MANAGER)
    shell->p_ctx->task_id = task_create(console_task, shell->p_name,(void *)shell);
    if (shell->p_ctx->task_id == TASK_ID_INVALID)
    {
        return NRF_ERROR_NO_MEM;
    }
    else
...
#if NRF_MODULE_ENABLED(SHELL_HISTORY)
    history_list_free_memory(shell);
#endif

    memset(shell->p_ctx, 0, sizeof(shell_ctx_t));
    shell->p_ctx->state = SHELL_STATE_UNINITIALIZED;
...
ret_code_t shell_uninit(shell_t const * shell)
{
    ...
    task_events_set(shell->p_ctx->task_id, SHELL_KILL_TASK_EVT);
    return NRF_SUCCESS;
#else
...
ret_code_t shell_start(shell_t const * shell)
{
    ...
    if (shell->p_ctx->state != SHELL_STATE_INITIALIZED)
    {
    ...
    ((shell_log_backend_t *)shell->p_log_backend->p_ctx)->p_context = p_context;
#endif

    ret_code_t err_code = shell->p_iface->p_api->enable(shell->p_iface, false);
...
void shell_process(shell_t const * shell)
{
    ...
    (void)nrf_atomic_u32_or((nrf_atomic_u32_t *)&shell->p_ctx->internal.value,
                            internal.value);

    switch (shell->p_ctx->state)
    {
    ...
    (void)nrf_atomic_u32_and((nrf_atomic_u32_t *)&shell->p_ctx->internal.value,
                             internal.value);
}

/* Function shall be only used by the nrf_fprintf module. */
void shell_print_stream(void const * p_user_ctx, char const * p_data, size_t data_len)
{
    cli_write((shell_t const *)p_user_ctx,
              p_data,
              data_len,
              NULL);
}

void shell_fprintf(shell_t const *      shell,
                     shell_vt100_color_t  color,
                     char const *           p_fmt,
                                            ...)
{
    ASSERT(p_fmt);
...
#if NRF_MODULE_ENABLED(SHELL_VT100_COLORS)
    if ((shell->p_ctx->internal.flag.use_colors) &&
        (color != shell->p_ctx->vt100_ctx.col.col))
    {
 ...
 */
static void format_offset_string_print(shell_t const * shell,
                                       char const *      p_str,
                                       ...
                                       bool              offset_first_line)
{
    ...
    while (1)
    {
        ...
        if (length <= shell->p_ctx->vt100_ctx.cons.terminal_wid - terminal_offset)
        {
            for (idx = 0; idx < length; idx++)
            {
                ...
                {
                    transport_buffer_flush(shell);
                    cli_write(shell, p_str + offset, idx, NULL);
                    offset += idx + 1;
                    cursor_next_line_move(shell);
                    cursor_right_move(shell, terminal_offset);
                    break;
        ...
        else
        {
            /* String is longer than terminal line so text needs to divide in the way 
               to not divide words. */
            length = shell->p_ctx->vt100_ctx.cons.terminal_wid - terminal_offset;
            ...
            while (1)
            {
                ...
                if ((idx + terminal_offset) >= shell->p_ctx->vt100_ctx.cons.terminal_wid)
                {
                    /* End of line reached. */
                    break;
                }
                        ...
                        size_t                          opt_len)
{
    ...
    shell_fprintf(shell,
                    SHELL_NORMAL,
                    "%s%s",
                    shell->p_ctx->p_current_stcmd->p_syntax,
                    cmd_sep);

    field_width = cli_strlen(shell->p_ctx->p_current_stcmd->p_syntax) + cli_strlen(cmd_sep);
    format_offset_string_print(shell, shell->p_ctx->p_current_stcmd->p_help, field_width, false);
    ...
    format_offset_string_print(shell, "Show command help.", field_width, false);
    ...
    if (p_opt != NULL)
    {
        for (size_t i = 0; i < opt_len; ++i)
        {
            ...
            {
                format_offset_string_print(shell, p_opt[i].p_optname_help, field_width, false);
            }
            else
            {
                cursor_next_line_move(shell);
    ...
    if (shell->p_ctx->p_current_stcmd->p_subcmd == NULL)
    {
        return;
    }
    ...
    shell_static_entry_t static_entry;
    shell_cmd_entry_t const * p_cmd = shell->p_ctx->p_current_stcmd->p_subcmd;
    shell_static_entry_t const * p_st_cmd = NULL;
    ...
    while (1)
    {
        cmd_get(p_cmd, !SHELL_CMD_ROOT_LVL, cmd_idx++, &p_st_cmd, &static_entry);
    ...
    while (1)
    {
        ...
        field_width = longest_string + tab_len;
        shell_fprintf(shell, SHELL_NORMAL,"  %-*s:", field_width, p_st_cmd->p_syntax);
        field_width += tab_len + 1; /* tab_len + 1 == "  " and ':' from: "  %-*s:" */

        if (p_st_cmd->p_help != NULL)
        {
            format_offset_string_print(shell, p_st_cmd->p_help, field_width, false);
        }
        else
        {
            cursor_next_line_move(shell);
...
static bool cli_log_entry_process(shell_t const * shell, bool skip)
{
    ...
    if (nrf_queue_pop(((shell_log_backend_t *)shell->p_log_backend->p_ctx)->p_queue, &entry) !=
                                                                                      NRF_SUCCESS)
    {
        return false;
    }
...
#if NRF_MODULE_ENABLED(SHELL_STATISTICS)
        ++shell->p_ctx->statistics.log_lost_cnt;
        if ((shell->p_ctx->statistics.log_lost_cnt & SHELL_LOG_MSG_OVERFLOW_MSK) == 1)
        {
    ...
    }
    {
        ...
        if (p_cons->cur_x > SHELL_INITIAL_CURS_POS)
        {
            cursor_left_move(shell, p_cons->cur_x - SHELL_INITIAL_CURS_POS);
        }
        cli_clear_eos(shell);
    }
    ...
    do
    {
        ...
        else if (header.base.generic.type == HEADER_TYPE_HEXDUMP)
        {
            ...
            do
            {
                ...
                data_len      -= chunk_len;
                nrf_log_hexdump_entry_process(data_buf, chunk_len, &params, shell->p_fprintf_ctx);
            } while (data_len > 0);
        }

        nrf_memobj_put(entry);
    } while (nrf_queue_pop(((shell_log_backend_t *)shell->p_log_backend->p_ctx)->p_queue, &entry)
                                                                                    == NRF_SUCCESS);
    return true;
}
...
{
    shell_log_backend_t * p_backend_cli = (shell_log_backend_t *)p_backend->p_ctx;
    shell_t const * shell = p_backend_cli->shell;

    //If panic mode cannot be handled, stop handling new requests.
    if (shell->p_ctx->state != SHELL_STATE_PANIC_MODE_INACTIVE)
    {
        bool panic_mode = (shell->p_ctx->state == SHELL_STATE_PANIC_MODE_ACTIVE);
        ...
        {
            (void)cli_log_entry_process(shell, panic_mode ? false : true);
...
static void nrf_log_backend_cli_panic_set(nrf_log_backend_t const * p_backend)
{
    ...
    if (shell->p_iface->p_api->enable(shell->p_iface, true) == NRF_SUCCESS)
    {
        shell->p_ctx->state = SHELL_STATE_PANIC_MODE_ACTIVE;
    }
    else
    {
        shell->p_ctx->state = SHELL_STATE_PANIC_MODE_INACTIVE;
    }
}
...
static void shell_cmd_colors_off(shell_t const * shell, size_t argc, char **argv)
{
    ...
    shell->p_ctx->internal.flag.use_colors = 0;
}
...
static void shell_cmd_colors_on(shell_t const * shell, size_t argc, char **argv)
{
    ...
    shell->p_ctx->internal.flag.use_colors = 1;
}
...
static void shell_cmd_history(shell_t const * shell, size_t argc, char **argv)
{
    ...
    size_t i = 0;
    nrf_memobj_t const * p_cmd_list = shell->p_ctx->p_cmd_list_tail;
    shell_memobj_header_t header;

    while (1)
    {
        if ((p_cmd_list == NULL) || (i >= SHELL_HISTORY_ELEMENT_COUNT))
        {
            break;
        }
...
void shell_cmd_cli_stats_show(shell_t const * shell, size_t argc, char **argv)
{
    ...
    nrf_queue_t const  * p_queue = ((shell_log_backend_t *)shell->p_log_backend->p_ctx)->p_queue;
...
void shell_cmd_cli_stats_reset(shell_t const * shell, size_t argc, char **argv)
{
    ...
    shell->p_ctx->statistics.log_lost_cnt = 0;
    nrf_queue_max_utilization_reset(
                               ((shell_log_backend_t *)shell->p_log_backend->p_ctx)->p_queue);
}
...
static void shell_cmd_resize(shell_t const * shell, size_t argc, char **argv)
{
    ...
    if (argc == 1)
    {
        if (terminal_size_get(shell,
                              &shell->p_ctx->vt100_ctx.cons.terminal_wid,
                              &shell->p_ctx->vt100_ctx.cons.terminal_hei) != NRF_SUCCESS)
        {
            shell->p_ctx->vt100_ctx.cons.terminal_wid = SHELL_DEFAULT_TERMINAL_WIDTH;
            shell->p_ctx->vt100_ctx.cons.terminal_hei = SHELL_DEFAULT_TERMINAL_HEIGHT;
            shell_fprintf(shell,
