...
{
	shell->ctx->p_cmd_list_element = NULL;
}
...
static void history_handle(shell_t const *shell, bool up)
{
	...
	if (!up) /* button down */
	{
		if (shell->ctx->p_cmd_list_element == NULL)
		{
			return;
		}
		cursor_home_position_move(shell);

		nrf_memobj_read(shell->ctx->p_cmd_list_element,
				&header,
				SHELL_HISTORY_HEADER_SIZE,
				0);

		shell->ctx->p_cmd_list_element = header.p_next;
		current_cmd_len = shell->ctx->cmd_buff_len;

		if (shell->ctx->p_cmd_list_element == NULL)
		{
			if (cli_strlen(shell->ctx->temp_buff) > 0)
			{
			...
			header.cmd_len = cli_strlen(shell->ctx->cmd_buff);
			skip = true;
		}
	}
	else /* button up */
	{
		if ((shell->ctx->p_cmd_list_element == shell->ctx->p_cmd_list_tail) ||
		...
		if (shell->ctx->p_cmd_list_element == NULL)
		{
			current_cmd_len = cli_strlen(shell->ctx->cmd_buff);

			shell->ctx->p_cmd_list_element = shell->ctx->p_cmd_list_head;
			/* Save the currently entered and not executed command. */
			if (current_cmd_len > 0)
			{
				strcpy(shell->ctx->temp_buff, shell->ctx->cmd_buff);
			}
			else
			{
				shell->ctx->temp_buff[0] = '\0';
		...
		else
		{
			...
			shell->ctx->p_cmd_list_element = header.p_prev;
		}
	}
	if (!skip)
	{
	...
	shell->ctx->cmd_buff_pos = header.cmd_len;
	shell->ctx->cmd_buff_len = header.cmd_len;
...
static void history_list_element_add(shell_t const *shell, nrf_memobj_t * p_memobj)
{
	...
	if (shell->ctx->p_cmd_list_head == NULL)
	{
		shell->ctx->p_cmd_list_head = p_memobj;
		shell->ctx->p_cmd_list_tail = p_memobj;
		header.p_prev = NULL;
		header.p_next = NULL;
		header.cmd_len = shell->ctx->cmd_buff_len;
	}
	else
	{
		nrf_memobj_read(shell->ctx->p_cmd_list_head,
				&header,
				SHELL_HISTORY_HEADER_SIZE,
				0);
		...
		nrf_memobj_write(shell->ctx->p_cmd_list_head,
				&header,
				SHELL_HISTORY_HEADER_SIZE,
				0);

		header.p_next = NULL;
		header.p_prev = shell->ctx->p_cmd_list_head;
		header.cmd_len = shell->ctx->cmd_buff_len;
	...
	nrf_memobj_write(p_memobj,
			shell->ctx->cmd_buff,
			shell->ctx->cmd_buff_len + 1, /* +1 for '\0' */
			SHELL_HISTORY_HEADER_SIZE);
}

static void history_list_element_oldest_remove(shell_t const *shell)
{
	if (shell->ctx->p_cmd_list_tail == NULL)
	{
		return; // nothing to do
	}

	shell_memobj_header_t header;
	nrf_memobj_t * p_memobj = shell->ctx->p_cmd_list_tail;
	...
	shell->ctx->p_cmd_list_tail = header.p_next;
	...
	nrf_memobj_read(shell->ctx->p_cmd_list_tail,
			&header,
			SHELL_HISTORY_HEADER_SIZE,
			0);

	header.p_prev = NULL;
	nrf_memobj_write(shell->ctx->p_cmd_list_tail, &header, SHELL_HISTORY_HEADER_SIZE, 0);
}

static void history_list_free_memory(shell_t const *shell)
{
	while (shell->ctx->p_cmd_list_tail != NULL)
	{
...
{
	shell_cmd_len_t cmd_new_len = cli_strlen(shell->ctx->cmd_buff);
	...
	if (shell->ctx->p_cmd_list_head != NULL)
	{
		...
		if (cmd_new_len == header.cmd_len)
		{
			...
			if (strcmp(shell->ctx->cmd_buff, shell->ctx->temp_buff) == 0)
			{
