...
void shell_help_print(const struct shell *shell,
	...
	field_width = shell_strlen(shell->ctx->active_cmd.syntax)
			+ shell_strlen(cmd_sep);
	format_offset_string_print(shell, shell->ctx->active_cmd.help,
			field_width, false);
	...
	struct shell_cmd_entry const *cmd = shell->ctx->active_cmd.subcmd;
