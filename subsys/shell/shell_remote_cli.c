...
{
	struct shell_remote_cli *sh = priv;
...
static const struct ipc_ept_cfg ep_cfg = {
	.name = CONFIG_SHELL_REMOTE_CLI_EP_NAME,
	.cb = {
		.bound    = ep_bound,
		.received = ep_recv,
	},
...
{
	struct shell_remote_msg_result rsp = {
                                ...
                                void *pkg, size_t len)
{
	...
	static const uint32_t cpy_flags = CBPRINTF_PACKAGE_CONVERT_RW_STR |
		(IS_ENABLED(CONFIG_SHELL_REMOTE_CLI_COPY_RO_STRINGS) ?
	...
	if (total_len > CONFIG_SHELL_REMOTE_CLI_BUF_SIZE) {
		...
		if (IS_ENABLED(CONFIG_SHELL_REMOTE_CLI_COPY_RO_STRINGS)) {
			...
			CBPRINTF_STATIC_PACKAGE(pkg_buf, sizeof(pkg_buf), len,
				SHELL_REMOTE_CLI_ALIGN, cpy_flags,
				"Buffer size (%d) too small (%d needed)",
				CONFIG_SHELL_REMOTE_CLI_BUF_SIZE, total_len);
			__ASSERT_NO_MSG(len > 0);
			len = cbprintf_package_copy(pkg_buf, len, fsc_pkg, pkg_len,
					    cpy_flags, NULL, 0);
			...
			CBPRINTF_STATIC_PACKAGE(fsc_pkg, pkg_len, len, SHELL_REMOTE_CLI_ALIGN,
				cpy_flags, "Buffer size (%d) too small (%d needed)",
				CONFIG_SHELL_REMOTE_CLI_BUF_SIZE, total_len);
		}
		total_len = len + offsetof(struct shell_remote_msg_print, data) + 8;
	} else {
		err = cbprintf_package_copy(pkg, len, fsc_pkg, pkg_len,
					    cpy_flags, strl, ARRAY_SIZE(strl));
	...
	#ifdef CONFIG_MULTITHREADING
		err = k_sem_take(&sh_remote->sem, K_MSEC(1000));
		if (err < 0) {
...
static void cmd_get(struct shell_remote_cli *sh_remote,
		    const struct shell_remote_msg_cmd_get *msg,
		    size_t len)
{
...
static void cmd_exec(struct shell_remote_cli *sh_remote,
...
#else
	k_thread_create(&sh_remote->thread, stack, K_KERNEL_STACK_SIZEOF(stack),
			thread_func, sh_remote, NULL, NULL, K_LOWEST_APPLICATION_THREAD_PRIO, 0,
			K_NO_WAIT);
...
#ifdef CONFIG_MULTITHREADING
	k_sem_init(&shell_remote.sem, 0, 1);
#else
	shell_remote.bounded = false;
#endif
