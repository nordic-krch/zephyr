...
static void uart_rx_handle(const struct shell_uart *sh_uart)
{
	...
	do {
		len = uart_fifo_read(sh_uart->ctrl_blk->dev, buf, sizeof(buf));

		if (len) {
			sys_ring_buf_raw_put(sh_uart->rx_ringbuf,
					     buf, len);
			new_data = true;
		}
	} while (len);
...
static void uart_tx_handle(const struct shell_uart *sh_uart)
{
	...
	len = sys_ring_buf_raw_get(sh_uart->tx_ringbuf, sh_uart->txbuf,
				   SHELL_UART_TX_BUF_SIZE);

	if (len) {
		int done;

		done = uart_fifo_fill(dev, sh_uart->txbuf, len);
		/* We cannot do anything if uart have not taken all data. */
		if (done != len) {
			LOG_WRN("TX: %d bytes sent, %d requested.", done, len);
		}
	} else {
...
{
	*cnt = sys_ring_buf_raw_put(sh_uart->tx_ringbuf, data, length);
