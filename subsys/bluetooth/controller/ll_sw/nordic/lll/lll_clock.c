...
#else
	return nrf_clock_control_request(DEVICE_DT_GET_ONE(nordic_nrfx_clock_lfclk), NULL, &lf_cli);
#endif
}
...
#else
	return nrf_clock_control_cancel_or_release(DEVICE_DT_GET_ONE(nordic_nrfx_clock_lfclk), NULL,
						   &lf_cli);
#endif
}
...
#else
	err = blocking_on(DEVICE_DT_GET_ONE(nordic_nrfx_clock_lfclk), LFCLOCK_TIMEOUT_MS);
#endif
	if (err) {
		return err;
	}
...
#else
	err = nrf_clock_control_release(DEVICE_DT_GET_ONE(nordic_nrfx_clock_lfclk), NULL);
#endif
...
#else
	return DT_ENUM_IDX(DT_NODELABEL(lfclk), k32src_accuracy_ppm);
#endif
}
...
#else
	return sca_ppm_lut[DT_ENUM_IDX(DT_NODELABEL(lfclk), k32src_accuracy_ppm)];
#endif
}
